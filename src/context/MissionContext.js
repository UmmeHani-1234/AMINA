...
export const MissionProvider = ({ children }) => {
  ...
  const [environment, setEnvironment] = useState(() => {
    try {
      return window.localStorage.getItem("amina-environment") || "night";
    } catch {
      return "night";
    }
  });
  ...
  const value = useMemo(() => ({ scenario, setScenario, current, stage, events, progress, running, setRunning, environment, setEnvironment: changeEnvironment, scenarioData }), [scenario, current, stage, events, progress, running, environment, changeEnvironment]);
